...
def simulate_singlewell_step(model, time, warm_start_t, delta_t, Uk, time_series=None):
    ...
    _ = model.do_step(time + time_offset, delta_t) # Perform 1-step simulation of delta_t from time
...
def step_input(start_time, stop_time, step_time, delta_t=1, start_val=0, end_val=1):
    start = start_time // delta_t
    stop = stop_time // delta_t
    step = step_time // delta_t
    timeline = np.linspace(start, stop + 1, num=stop-start)
    input_profile = start_val + end_val * np.heaviside(timeline - step, 1)
...
if __name__ == '__main__':
    for main_run_nr in range(0, 1):
        ...
        choke = 50 # range: [0, 100]   [%]
        choke_bounds = [0,100]
        GL = 7500  # range: [0, 10000] [m^3/hr]
        ...
        while time < final_time:
            y1k, y2k = simulate_singlewell_step(model, time, warm_start_t, delta_t, input_profile[:, itr])
            y1.append(y1k)
            y2.append(y2k)
            print(f'itr nr. {itr}\n')
