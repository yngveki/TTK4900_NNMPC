...
def plot_mse_specific_test(n_models, test_name, **kwargs):
    x_axis = np.linspace(0, n_models, n_models)
    # -- PLOTTING -- #
    fig, ax = plt.subplots(1,1)
    ax.set_title(f'MSE when testing model on \'{test_name}\'')
    for test_name, mse in kwargs.items():
        ax.plot(x_axis, mse, '-', label=f'MSE, \'{test_name}\'')
...
if __name__ == '__main__':
    csv_path = Path(__file__).parent / 'models/model_grid_mses.csv'
